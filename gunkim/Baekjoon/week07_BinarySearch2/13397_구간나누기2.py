...
def divideSection(value): # 투포인터 알고리즘 활용이다. 2개의 포인터 max,min을 통해 구간의 최대차이를 만든다. 이를 value와 비교한다.
    # 첫 번째 구간 설정
    ...
    for i in range(1, N):
        ...
        if max_val - min_val > value: # num[i]를 구간에 포함시키면서 value를 넘기 때문에 새로운 구간을 만들어 num[i] 부터 구간을 시작한다.
            cnt += 1
            max_val = num[i]
            min_val = num[i]
    return cnt
